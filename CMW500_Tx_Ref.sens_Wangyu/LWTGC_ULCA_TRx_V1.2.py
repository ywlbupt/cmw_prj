...
LteTdd_measure=1               # 是否测量LTE FDD，1为enable，0为disable
LteFdd_CULCA_measure=0         # 是否测量LTE FDD连续CA，1为enable，0为disable。只测ULCA ACLR，不测灵敏度
LteTdd_CULCA_measure=0         # 是否测量LTE TDD连续CA，1为enable，0为disable。只测ULCA ACLR，不测灵敏度
WCDMA_measure=10              # 是否测量WCDMA制式 
...
GSMcall_mode=0             # 决定从哪一端发起GSM呼叫。见上面说明。
GSMAttachWaitTime=200       # 有SIM卡时，等待手机attach的最长时间，单位是秒
...
CLTPower=-20                # closed loop target power闭环功控的目标功率，单位是dBm。对所有制式适用。
...
LteBW="B100"               # B014 B030 B050 B100 B200分别代表LTE带宽为1.4/3/5/10/20MHz。目前只支持5/10/20MHz选项
...
ULpartialRB=1              # 为1时，接收机测试时b5、8、20等窄频段按照3GPP要求，设置上行为20/25个partial RB。为0则都按Lte_UL/DLRB设置。
...
UlRB_CULCA="N100"          # 上行连续CA的下行RB设置。一般可能设为100或1
...
ULRB_PCC_position="LOW"    # PCC上行RB的位置，需按照3GPP《36521-1-f31_s06a-s06d》p320~323确定。
ULRB_SCC_position="HIGH"   # SCC上行RB的位置，需按照3GPP《36521-1-f31_s06a-s06d》p320~323确定。
ULCA_instr_version="NEW"   # 取值为"NEW"或"OLD"。仪表版本有新旧2种，对ULCA测量的处理流程不一样。如果频繁掉线，请尝试换另一个。



######################### Band definition   ##############################################
#bandLteFdd = ["OB1", "OB2", "OB3", "OB4", "OB5", "OB7", "OB8", "OB20", "OB28"]   #D2T频段
#bandLteFdd = ["OB1", "OB3", "OB5", "OB7", "OB8"]   #D2T频段
bandLteFdd = ["OB1", "OB3", "OB5", "OB8"]   #D2T频段

bandLteTdd = ["OB40", "OB41"] 
bandLteTdd = ["OB34", "OB38", "OB39", "OB40", "OB41"] 

#bandLteFdd_CULCA = ["OB3", "OB7","OB3", "OB7","OB3", "OB7","OB3", "OB7","OB3", "OB7","OB3", "OB7"]
bandLteFdd_CULCA = ["OB3"]
bandLteTdd_CULCA = ["OB40"]
#bandLteTdd_CULCA = ["OB38", "OB39", "OB40", "OB41"] 

#bandWCDMA = ["OB1", "OB2", "OB4", "OB5", "OB8"]
bandWCDMA = ["OB1", "OB2", "OB5", "OB8"]
bandWCDMA = ["OB1", "OB5", "OB8"]
...
if RepeatFlag==1:
    bandtemp = []
    for temp in range(3):
        #bandtemp += bandGSM               #把一个序列复制n遍以做老化测试。
...
def main():
    ...
    time.sleep(2)
    # 配置线损
    lossName = PMqueryWithDelay(PM,3,"CONFigure:BASE:FDCorrection:CTABle:CATalog?")
    if lossName.find ("CMW_loss") != -1:
        PMwrite(PM,"CONFigure:BASE:FDCorrection:CTABle:DELete 'CMW_loss'")
    PMwrite(PM,"CONFigure:BASE:FDCorrection:CTABle:CREate 'CMW_loss', 700000000, 0.5, 960000000, 0.5, \
        1710000000, 0.8, 2200000000, 0.8, 2300000000, 0.9, 2690000000, 0.9, 3300000000, 1.5, 4400000000, 1.8,\
        5000000000, 2, 5825000000, 2.4")
    #PMwrite(PM,"CONFigure:BASE:FDCorrection:CTABle:CREate 'CMW_loss', 700000000, 1, 960000000, 1, \
    #    1710000000, 1.6, 2200000000, 1.6, 2300000000, 1.8, 2690000000, 1.8, 3300000000, 3, 4400000000, 3.6,\
    #    5000000000, 4, 5825000000, 4.8")
...
############################    LTE ACLR & Rx_Sens.lev test ##################################################
    if (LteFdd_measure==1 or LteTdd_measure==1):
        ...
        print("LTE bandwidth is %s, ULRB number is %s" %(LteBW,Lte_UlRB))
        time.sleep(2)
        ...
        #if UEPwrCtrl == "MAXPower":                                           # 设置参考电平，会影响到ACLR数值。但认证机构应该会用ULPC？
        #    PMwrite(PM,"CONFigure:LTE:SIGN:RFSettings:ENPMode MANual")
        #    PMwrite(PM,"CONFigure:LTE:SIGN:RFSettings:ENPower 24")
        #    PMwrite(PM,"CONFigure:LTE:SIGN:RFSettings:UMARgin 12")
        #elif (UEPwrCtrl == "CLOop" and CLTPower > 18):                        # 设置参考电平，会影响到ACLR数值。但认证机构应该会用ULPC？
        ...
        for i in range(len(bandLte)):
            ...
            for j in range(len(channelLte[i])):
                ...
                while 1:
                    try:
                        #LteAclr = PMqueryWithDelay(PM,3,"READ:LTE:MEAS:MEValuation:ACLR:AVERage?")
                        PMwrite(PM,"INIT:LTE:MEAS:MEValuation")
                        while PMquery(PM,"FETCh:LTE:MEAS:MEValuation:STATe?").strip()!="RDY":
                            time.sleep(0.2)
                        LteAclr = PMquery(PM,"FETCh:LTE:MEAS:MEValuation:ACLR:AVERage?")
                ...
                if Rx_measure==1:
                    ...
                    (RSepre,LteFCPwr,LteBler,RSRP,RSRQ)=SeekLteSens(PM,UpperLimitLte,CoarseStepLte,FineStepLte,2)
...
############################    LTE Contiguous ULCA ACLR test ##################################################
    if (LteFdd_CULCA_measure==1 or LteTdd_CULCA_measure==1):
        ...
        time.sleep(0.5)
        PMwrite(PM,"ROUTe:LTE:SIGN:SCENario:CATRfout:FLEXible SUA1,RF1C,RX1,RF1C,TX1,SUA2,RF1C,TX3")
        time.sleep(0.5)
        ...
        if PMquery(PM,"ROUTe:LTE:SIGN:SCENario?") != 'CATR\n':
            input("请检查仪表scenario 2CC设置是否成功，手工设置后按Enter继续");
        ...
        print("band channel  txpwr  UTRA2  UTRA1  E-UTRA1 E-UTRA1 UTRA1  UTRA2")
        LogfileWrite(LogFile, "band channel  txpwr  UTRA2  UTRA1  E-UTRA1 E-UTRA1 UTRA1  UTRA2\n")
        ...
        for i in range(len(bandLte_CULCA)):
            ...
            if LteFdd_CULCA_measure==1 and i==len(bandLteFdd_CULCA):  #Fdd到Tdd切换时：
                #os.popen ("adb reboot")
                PMwrite(PM,"SOURce:LTE:SIGN:CELL:STATe OFF")
                PMwrite(PM,"CONFigure:LTE:SIGN:SCC:UUL OFF")   #新仪表在切换FDD/TDD之前，必须先取消SCC uplink enable
                PMwrite(PM,"CONFigure:LTE:SIGN:DMODe TDD")
                ...
                PMwrite(PM,"CONFigure:LTE:SIGN:RFSettings:PCC:CHANnel:UL %d" %channelLte_CULCA[len(bandLteFdd_CULCA)][0])
                PMwrite(PM,"CONFigure:LTE:SIGN:DL:PCC:RSEPre:LEVel -80")
                PMwrite(PM,"CONFigure:LTE:SIGN:SCC:UUL ON,RF1C,RX3")
                PMwrite(PM,"CONFigure:LTE:SIGN:SCC:CAGGregation:MODE INTRaband")
                PMwrite(PM,"SOURce:LTE:SIGN:CELL:STATe ON")
            ...
            if bandLte_CULCA[i] == "OB39":       #对于B39做特殊处理
                ...
                PMwrite(PM,"CONFigure:LTE:SIGN:RFSettings:PCC:CHANnel:UL %d" %channelLte_CULCA[i][0])
            else:
                PMwrite(PM,"CONFigure:LTE:SIGN:CELL:BANDwidth:SCC:DL %s" %LteBW_CULCA)
            for j in range(len(channelLte_CULCA[i])):
                ...
                if ULCA_instr_version == "OLD":
                    PMwrite(PM,"CONFigure:LTE:SIGN:SCC:CAGGregation:MODE INTRaband")
                    PMwrite(PM,"CALL:LTE:SIGN:SCC:ACTion:MACactivate")
                time.sleep(1)
                PMwrite(PM,"CONFigure:LTE:SIGN:CONNection:PCC:RMC:DL %s,%s,KEEP" %(DlRB_CULCA_measure, LteDlModType))
                PMwrite(PM,"CONFigure:LTE:SIGN:CONNection:PCC:RMC:RBPosition:UL %s" %ULRB_PCC_position)
                PMwrite(PM,"CONFigure:LTE:SIGN:CONNection:SCC:RMC:DL %s,%s,KEEP" %(DlRB_CULCA_measure, LteDlModType))
                PMwrite(PM,"CONFigure:LTE:SIGN:CONNection:SCC:RMC:RBPosition:UL %s" %ULRB_SCC_position)
                PMwrite(PM,"ABORt:LTE:SIGN:EBLer") #在20MHz BW下，测完Rx需设置BLER OFF，否则后续的ACLR测量出错
                PMwrite(PM,"CONFigure:LTE:SIGN:UL:PUSCh:TPC:SET MINPower")       #在min/max之间至少切换一次，否则可能overdriven
                PMwrite(PM,"ABORt:LTE:MEAS:MEValuation")
                time.sleep(0.2)
                PMwrite(PM,"CONFigure:LTE:SIGN:UL:PUSCh:TPC:SET MAXPower")       #在min/max之间至少切换一次，否则可能overdriven
                time.sleep(0.2)
                PMwrite(PM,"CONFigure:LTE:SIGN:UL:PUSCh:TPC:SET %s" %UEPwrCtrl)  #在min/max之间至少切换一次，否则可能overdriven
                while 1:
                    try:
                        LteAclr = PMqueryWithDelay(PM,2,"READ:LTE:MEAS:MEValuation:ACLR:CURRent?")
                        listLteAclr = list(map(float,LteAclr.split(',')))
                        break
                    except Exception:
                        input("read LteAclr error");
                        PMwrite(PM,"ABORt:LTE:MEAS:MEValuation")
                        time.sleep(1)
                print("%-4s  %d   %3.2f  %3.2f  %3.2f  %3.2f   %3.2f   %3.2f  %3.2f" \
    ...
    if WCDMA_measure==1:
        ...
        for i in range (len(bandWCDMA)):
            ...
            for j in range(len(channelWCDMA[i])):
                ...
                time.sleep(0.4)
                PMwrite(PM,"INIT:WCDMa:MEAS:MEValuation")
                while PMquery(PM,"FETCh:WCDMa:MEAS:MEValuation:STATe?").strip()!="RDY":
                    time.sleep(0.2)
                WAclr = PMquery(PM,"FETCh:WCDMa:MEAS:MEValuation:SPECtrum:AVERage?").split(',')
                while WAclr[15] in ["INV", "NAV", "NCAP"]:
                    print("TxPwr=NULL, maybe call lost. Please wait for reconnect")
                    #os.popen ("adb reboot")
                    #tkinter.messagebox.showinfo("提示","WCDMA连接断，请等待自动连接");
                    PMwrite(PM,"SOURce:WCDMa:SIGN:CELL:STATe OFF")
                    time.sleep(2)
                    PMwrite(PM,"SOURce:WCDMa:SIGN:CELL:STATe ON")
                    time.sleep(3)
                    ...
                    while PMquery(PM,"FETCh:WCDMa:MEAS:MEValuation:STATe?").strip()!="RDY":
                        time.sleep(0.2)
                    WAclr = PMquery(PM,"FETCh:WCDMa:MEAS:MEValuation:SPECtrum:AVERage?").split(',')
                    #WAclr = PMqueryWithDelay(PM,3,"READ:WCDMa:MEAS:MEValuation:SPECtrum:AVERage?").split(',')
    ...
    if TD_measure==1:
        ...
        for i in range (len(bandTDS)):
            ...
            for j in range(len(channelTDS[i])):
                ...
                time.sleep(4.5)               # 切信道需要的延时较大，否则未执行完毕时，下一个命令可能不生效
                if RF_output=="RF1O":
                    PMwrite3G_DLPwr(PM,"TDSCdma",-59)  # TD连接状态下切天线之前需要把功率打高，因为TD切天线需要大约2秒钟，低功率会导致掉线
                    time.sleep(1.5)
                ...
                while 1:
                    ...
                    time.sleep(1)
                    TDSAclr = PMqueryWithDelay(PM,3,"READ:TDSCdma:MEAS:MEValuation:SPECtrum:AVERage?").split(',')
                    if TDSAclr[0] =="0":
                        break
                    else:
                        time.sleep(4)
                        TDSAclr = PMqueryWithDelay(PM,4,"READ:TDSCdma:MEAS:MEValuation:SPECtrum:AVERage?").split(',')
                        if TDSAclr[0] =="0":
                            break
                        else:
                            print("TxPwr=NULL, maybe call lost. Please wait for reconnect")
                ...
                if Rx_measure==1:
                    PMwrite(PM,"CONFigure:TDSCdma:SIGN:BER:TBLocks 100")
...
##############################     GSM Spectrum Switching & Rx_Sens.lev test   ##########################
    if GSM_measure==1:
        ...
        time.sleep(1)
        PMwrite(PM,"CONFigure:GSM:SIGN:RFSettings:LEVel:BCCH -70")
        PMwrite(PM,"CONFigure:GSM:SIGN:CELL:PSWitched:PDPContext REJect")
        ...
        PMwrite(PM,"CONFigure:GSM:SIGN:BER:CSWitched:MMODe BBB")
                     #如果设置成Burst by Burst，返回值位置在[2]。RBER/FER、BER mode返回值在[4]
        ...
        for i in range(len(bandGSM)):
            ...
            for j in range(len(channelGSM[i])):
                ...
                time.sleep(1)
                try:
                    PMwrite(PM,"INIT:GSM:MEAS:MEValuation")
                    while PMquery(PM,"FETCh:GSM:MEAS:MEValuation:STATe?").strip()!="RDY":
                        time.sleep(0.2)
                    GSSwitchResults = PMquery(PM,"FETCh:GSM:MEAS:MEValuation:SSWitching:FREQuency?")
                except Exception:
                    print("READ:GSM:MEAS:MEValuation:SSWitching:FREQuency timeout")
                    PMwrite(PM,"ABORt:GSM:MEAS:MEValuation")
                    time.sleep(2)
                    GSSwitchResults = PMqueryWithDelay(PM,3,"READ:GSM:MEAS:MEValuation:SSWitching:FREQuency?")
